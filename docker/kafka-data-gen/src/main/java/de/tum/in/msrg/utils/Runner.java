...
//        String topic = cmdLine.getOptionValue("topic","input");
        ...
        Gauge unprocessedEventsGauge = Gauge.build("de_tum_in_msrg_pgv_unprocessed_events", "Unprocessed events").labelNames("key").register();
        Gauge unprocessedOutputsGauge = Gauge.build("de_tum_in_msrg_pgv_unprocessed_outputs", "Unprocessed outputs").labelNames("key").register();
        ...
        for (int i = 0; i < numProducer; i++) {
            Runnable datagen = () -> {
                ...
                try {
                    dataGen = new KafkaDataGen(benchmarkLength, bootstrap, delay, delayLength, eventsPerWindow, inputTimeMap,inputIdMap, recordsCounter, expectedCounter);
                    dataGen.start();
